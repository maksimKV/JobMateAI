...
const InterviewSimulatorPage = () => {
  ...
  const {
    ...
    restartInterview,
    // setSession is intentionally left unused but might be needed for future features
  } = useInterviewSession();
  ...
  const handleStartInterview = useCallback(async () => {
    if (!jobDescription.trim()) {
      setUiError('Please enter a job description');
      return;
    }
    ...
    try {
      ...
      setUiError(err instanceof Error ? err.message : 'Failed to start interview');
    }
  }, [jobDescription, interviewType, interviewLength, startInterviewSession]);
