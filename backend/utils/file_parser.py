...
SPECIAL_CASES_PATH = Path(__file__).parent.parent / 'data' / 'special_cases.json'
...
def load_special_cases() -> Dict[str, str]:
    """Load and return the special cases for skill matching."""
    try:
        with open(SPECIAL_CASES_PATH, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        logger.error(f"Failed to load special cases: {e}")
        return {}
